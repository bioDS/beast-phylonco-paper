...
def plot_figs(stats_path, true_path, mode):
    ...
    if mode == 0:
        ...
        plt.clim(0, clim)
        if max(z) > clim:
            print("Warning, max colorbar value %f exceeds %f" % (max(z), clim))
        plt.title("Tree length error (binary EM)")
    elif mode == 1:
        z = hpd_range
        plt.scatter(x=true_alphas, y=true_betas, c=z, s=20, alpha=0.5)
        # color bar limits
        ...
        if max(z) > clim:
            print("Warning, max colorbar value %f exceeds %f" % (max(z), clim))
        plt.title("Tree length HPD (binary EM)")
    else:
        z = color_values
        plt.scatter(x=true_alphas, y=true_betas, c=z, s=20, alpha=0.5)
        plt.set_cmap("viridis_r")
        plt.colorbar()
        plt.title("Tree length coverage (binary EM)")
    # axis formatting
    ...
    output_path = "../figures/binary_EM_treelength_error_%s.pdf" % mode_options[mode]
